...
#+begin_src sh
  dotnet new webapi -n ApiApp # without controller
  dotnet new webapi -controllers -n ApiApp # with controller
#+end_src
...
*** What is a .sln file?
A .sln (Solution) file in Microsoft .NET is basically a container for multiple projects.
...
#+begin_example
  Solution (HRMS.sln)
   ...
   ├── HRMS.Infrastructure (Database)
#+end_example

👉 Why it matters
- Groups multiple projects together
- Used by Visual Studio
- Helps in large/clean architecture

Here how to do it correctly:
#+begin_src bash
  mkdir HRMS
  cd HRMS

  dotnet new sln -n HRMS # create sln with name
  dotnet new webapi --use-controllers -n API # create webapi with controller
  dotnet sln add API # add API to sln
#+end_src
